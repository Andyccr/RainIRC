...
当前版本 **0.2**：默认 **TLS 1.3 互认** + **Ed25519 消息签名**。
...
身份存在 `~/.p2pirc/identity.json`（Windows 为 `%USERPROFILE%\.p2pirc\`），重启后复用。输入 `/whoami` 可查看完整 Peer ID、指纹和公钥。
...
节点会向 UDP 组播 `239.255.77.77:7776` 宣告自己。`/discover` 列出最近看到的邻居。**不会自动连接**；你仍需 `/connect`。

组播被墙时，手动 `/connect` 仍然可用。发现包本身未认证。
...
Current version **0.2**: **TLS 1.3 mutual auth** and **Ed25519 message signatures** by default.
...
Identity is stored in `~/.p2pirc/identity.json` (or `%USERPROFILE%\.p2pirc\` on Windows) and reused across restarts. `/whoami` prints the full Peer ID, fingerprint, and public key.
...
Peers periodically announce themselves on UDP multicast `239.255.77.77:7776`. `/discover` lists recently seen neighbors. The program **does not** auto-connect; you still `/connect` the address you want.

Discovery is best-effort and unauthenticated. If multicast is blocked, `/connect` still works.
...
- Authenticated discovery packets; `/msg` only reaches a directly connected peer
...
| 0.2 | TLS 1.3, message signatures, `/whoami` (current) |
